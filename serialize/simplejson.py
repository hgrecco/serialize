...
def dumps(obj):
    return json.dumps(all.traverse_and_encode(obj, trav_dict=trav_dict), cls=Encoder, tuple_as_array=True).encode(
        'utf-8')


def dumps_pretty(obj):
    return json.dumps(all.traverse_and_encode(obj, trav_dict=trav_dict), cls=Encoder, sort_keys=True,
                      tuple_as_array=True,
